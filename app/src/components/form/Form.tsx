...
const Form = (props: FormProps) => {
    ...
    const valueToColorDescriptor = (value: number, type: LevelType) => {
      const soundLevels = [20, 40, 60, 80, 120]; // in dB
      const busyLevels = [20, 40, 60, 80, 95]; // in % (subject to change)
      const colors = ["#7765AC", "#4D74B2", "#659360", "#FCB955", "#E95658"];
      const soundLabels = ["FAINT", "SOFT", "MODERATE", "LOUD", "VERY LOUD"];
      ...
      return (
        <Chip
          label={chosenLabels[chosenLabels.length - 1]}
          variant="filled"
          sx={{
    ...
    const soundChip = valueToColorDescriptor(soundLevel, "sound");
    const busyChip = valueToColorDescriptor(busyLevel, "busy");
